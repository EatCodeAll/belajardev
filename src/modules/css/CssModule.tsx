...
import { CSS_CONTENT } from '../../data/content';
...
const CssModule: React.FC<Props> = ({ onCompleteStep, completedSteps, onNavigate }) => {
  const [currentStepIdx, setCurrentStepIdx] = useState(() => {
    const firstUncompleted = CSS_CONTENT.findIndex(s => !completedSteps.includes(s.id));
    return firstUncompleted === -1 ? 0 : firstUncompleted;
  });
  const [showCongrats, setShowCongrats] = useState(() => {
    return completedSteps.length === CSS_CONTENT.length && CSS_CONTENT.length > 0;
  });

  const currentStep = CSS_CONTENT[currentStepIdx];
  const isCompleted = completedSteps.includes(currentStep.id);

  const handleNext = () => {
    if (currentStepIdx < CSS_CONTENT.length - 1) {
  ...
  if (showCongrats) {
    return (
      <Congrats 
        ...
        xpEarned={CSS_CONTENT.reduce((acc, curr) => acc + curr.quest.xp, 0)}
        onContinue={() => onNavigate('js')}
      ...
      <div className="flex flex-col gap-4 px-4">
        <div className="flex items-center justify-between gap-4">
          <div className="flex flex-col min-w-0">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">CSS Basics</span>
        ...
        <ProgressBar current={currentStepIdx + 1} total={CSS_CONTENT.length} />
      </div>
